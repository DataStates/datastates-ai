...
rpc_client::rpc_client(const std::vector<std::string> &servers, const std::vector<int>&provider_ids) :
...
void rpc_client::append_to_timestamp_map(timestamp_map_t &profile_time_stamps, timestamp_t &timestamp, std::string &function_string){
...
void rpc_client::append_to_timestamp_map(timestamp_map_t &profile_time_stamps, std::vector<uint64_t> &timestamp, std::string &function_string){
    if (profile_time_stamps.find(function_string) == profile_time_stamps.end())
...
bool rpc_client::store_meta(const digraph_t &g, const composition_t &comp, const float val_acc, timestamp_map_t &profile_time_stamps) {
...
composition_t& rpc_client::get_composition(const model_id_t &id, timestamp_map_t &profile_time_stamps) {
...
bool rpc_client::read_layers(const model_id_t &id, const vertex_list_t &layer_id, std::vector<segment_t> &segment_list, std::vector<uint64_t> &owners, timestamp_map_t &profile_time_stamps){
    struct req_info_t {
