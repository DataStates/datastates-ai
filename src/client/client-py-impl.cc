...
py_backend::py_backend(std::string servers, size_t size){
		...
		mem_resources.emplace_back(std::make_unique<std::pmr::monotonic_buffer_resource>(mem_buffer.get(),client_pinned_buffer_size, std::pmr::null_memory_resource()));
		mem_resources.emplace_back(std::make_unique<std::pmr::unsynchronized_pool_resource>(mem_resources.back().get()));
		mem_resources.emplace_back(std::make_unique<debug_resource>("pool", mem_resources.back().get(), mem_buffer.get()));
...
int py_backend::load_layers(tensor_list_t&tensors, uint64_t &model_id, uint64_list_t&layer_ids, uint64_list_t&layer_owners){
        std::pmr::vector<char>temp{mem_resources.back().get()};
        std::pmr::vector<char> ptrs[tensors.size()];
        std::vector<segment_t> segments;
        for(int i=0 ; i<tensors.size(); ++i)
...
bool py_backend::store_meta(uint64_t id, uint64_list_t& edges, int m, uint64_list_t& layer_ids, uint64_list_t& layer_owners, uint64_list_t& sizes, int n, const float val_acc){
...
int py_backend::get_composition(uint64_t id, uint64_list_t&layer_ids, uint64_list_t&layer_owners, int n){
