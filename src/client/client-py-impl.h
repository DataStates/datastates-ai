...
class py_backend {
	...
	int load_layers(tensor_list_t& tensors, uint64_t&model_ids, uint64_list_t&layer_ids, uint64_list_t&layer_owners);
	bool store_meta(uint64_t id, uint64_list_t&edges, int m, uint64_list_t&layer_ids, uint64_list_t&layer_owners, uint64_list_t&sizes, int n, const float val_acc);
